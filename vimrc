    ...
    elseif expand("%:e") == "python" || expand("%:e") == "py"
        ...
        if getftime(class) >= getftime(Sou)
            ...
            if g:iswindows
                ...
                if g:isGUI
                    exe ":!gnome-terminal -x bash -c 'python %<; echo; echo 请按 Enter 键继续; read'"
                else
                    exe ":!clear; python %<"
                endif
            endif
...
imap <F9> <ESC>:call RunPython()<CR>
vmap <F9> <ESC>:call RunPython()<CR>
autocmd BufRead *.py nmap <F10> :make<CR>  
...
"  < powerline 插件配置 >
" -----------------------------------------------------------------------------
" 状态 set guifont=PowerlineSymbols\ for\ Powerline
" -----------------------------------------------------------------------------
set laststatus=2
set t_Co=256
set encoding=utf8
...
highlight Cursor
            \ guifg=NONE guibg=purple
highlight iCursor
            \ guifg=NONE guibg=blue
highlight CursorIM
            \ guifg=NONE guibg=purple
set guicursor=n-v-c:hor20-Cursor-blinkon0
